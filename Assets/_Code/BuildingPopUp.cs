...
public class BuildingPopUp : MonoBehaviour {
    ...
    void InputDropdown(Dropdown change)                                                 //Triggered when the dropdown menu changes
    {
        if (SelectedBuildingSpecial == 1)                                                       //If this is a Gate
        {
            ...
            SelectedBuilding.GetComponent<BuildingOption>().SelectedOption = System.Convert.ToByte(DropDownMenu.value); //Push this update though to the Building itzelf (so we can save this state)
        }
    }
}
